...
Supabase free-project limits blocked a separate hosted test project, and the owner declined a paid branch. The repository uses a disposable Supabase setup in `integration/` and GitHub Actions to run actual Auth, Storage, PostgREST, and RLS checks on an Ubuntu runner. The migration was then applied to the live RealEstate database on 2026-09-24. Validation found 12 approved existing sales, 12 corresponding private contacts, zero public sale phone values, and two private evidence buckets. No staff reviewer has been provisioned yet; an authorized Clerk user ID must be entered into `sale_staff` before staff review can operate.
