                      ...
                      return (
                        <article className="my-listings-card" key={property.key}>
                          ...
                          <div className="my-listings-card-manage">
                            ...
                            {protectedSalesEnabled && property.property_type === "sale" && !property.archived_at && <div className="sale-listing-review"><strong>{saleCopy[property.review_status] || saleCopy.pending}</strong><AuthorityUpload propertyId={String(property.key)} />{property.review_status === "approved" && <button type="button" onClick={() => refreshSale(property)}>{saleCopy.refresh}</button>}</div>}
